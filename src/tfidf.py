...
def tokenize(post):
    ...
    for word in nltk.word_tokenize(post):
        if stemmer.stem(word) not in stop_words and word.isalnum() and not word.isdecimal():
            STEMMED_LIST[stemmer.stem(word)] = set(list(STEMMED_LIST.get(stemmer.stem(word), [])) + [word])
    return words
...
def compute_top_tfidf(df, output, categories, column):
    ...
    top_df = tfidf_df.apply(lambda s: s.abs().nlargest(10).index.tolist(), axis=1)
    ...
    for word in highest_dict.keys():
        print(word)
        for key in highest_dict[word].keys():
            print(f"Stemmer DICT for {key}: {STEMMED_LIST.get(key)}")
    #print(top_df)
